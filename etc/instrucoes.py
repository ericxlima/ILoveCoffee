...
def pedra_papel_tesoura():
    texto = 'O Jogo "Pedra Papel Tesousa" é exatamente como o clássico, cada vez que você ganhar um jogo contra a ' \
            'máquina, ganha 100 pontos. Quando perder, perde 50 pontos e em casos de empate, não ganha nada. O jogo' \
            'utiliza um sistema de pseudo-aleatoriedade, e não é do jeito que está, não é possível prever as jogadas' \
            'da máquina. Divirta-se jogando, nas suas vitórias e derrotas :)'
    return texto


def banco():
    texto = 'No Banco você pode fazer pequenos empréstimos com base no "Ranking do Rei do Café", além disso, você pode' \
           'fazer cartões de crédito, e realizar depósitos em contas de amigos/parceiros. O dinheiro pode ser visto ' \
           'como um item de "ostentação" no jogo ultimamente, futuramente adicionaremos novas funções para fazer com ' \
           'o dinheiro. Até lá, depositem o máximo que conseguirem :)'
    return texto

def jogo_da_velha():
    texto = 'O jogo da Velha é o primeiro projeto MultiPlayer do ILoveCoffe, o plano é fazer com que nele possamos ' \
            'logar dois players simultaneamente, e suas respectivas pontuações. Nele, 2 players poderão jogar PvP' \
            '(Player vs Player) até com que um saia vencedor ou dê velha. As pontuações serão definidas depois...'
    return texto
